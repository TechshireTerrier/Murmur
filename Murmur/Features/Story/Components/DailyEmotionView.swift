            ...
            if isLoading {
                ...
                HStack {
                    ForEach(0..<emotions.count, id: \.self) { index in
                        Text("\(emotions[index].label)")
                            .modifier(EmotionLabelModifier())
                            .accessibilityLabel("추출된 감정: \(emotions[index].label)")
...
#Preview("감정 있음") {
    DailyEmotionView(
        emotions: [
            EmotionResult(label: "기쁨", confidence: 0.85),
            EmotionResult(label: "감동", confidence: 0.72),
            EmotionResult(label: "뿌듯함", confidence: 0.68),
            EmotionResult(label: "설렘", confidence: 0.45)
        ],
        isLoading: false
    )
    .padding()
...
#Preview("로딩 중") {
    DailyEmotionView(emotions: [], isLoading: true)
        .padding()
}

#Preview("감정 없음") {
    DailyEmotionView(emotions: [], isLoading: false)
        .padding()
}
